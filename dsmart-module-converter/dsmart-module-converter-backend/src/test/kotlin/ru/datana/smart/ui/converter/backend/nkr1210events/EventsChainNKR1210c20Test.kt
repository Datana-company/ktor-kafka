...
internal class EventsChainNKR1210c20Test {

    /**
     * NKR-1210
     * Проверка изменения статуса рекомендации "Предупреждение" и обнуления данных
     * в зависимости от значений dataTimeout и meltTimeout
     * FAILED: пока не реализована задача NKR-1222
     */
    @Ignore
    @Test
    fun `steel warning should be inactive, data should be reset after dataTimeout`(){
        runBlocking {
    ...
    @Ignore
    @Test
    fun `slag warning should be inactive, data should be reset after dataTimeout`(){
        runBlocking {
            val timeStart = Instant.now()
            val dataTimeout = 5000L
