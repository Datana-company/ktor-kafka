...
suspend fun ConverterBeContext.eventSteelInfoReached(setActive: Boolean = true) = this.setEventReached(
        setActive = setActive,
        message = """
                   Достигнут предел потерь металла в потоке – ${currentStateRepository.lastAvgSlagRate().toPercent()}%.
                  """.trimIndent()){
             this.eventInfo()
    }
...
suspend fun ConverterBeContext.eventSteelWarningReached(setActive: Boolean = true) = this.setEventReached(
    setActive = setActive,
    message = """
              В потоке детектирован металл – ${currentStateRepository.lastAvgSlagRate().toPercent()}% сверх допустимой нормы ${this.streamRateWarningPoint.toPercent()}%. Верните конвертер в вертикальное положение.
               """.trimIndent()){
    this.eventWarning()
}
...
suspend fun ConverterBeContext.eventSteelCriticalReached(setActive: Boolean = true) = this.setEventReached(
    setActive = setActive,
    message = """
              В потоке детектирован металл – ${currentStateRepository.lastAvgSlagRate().toPercent()}%, процент потерь превышает критическое значение – ${this.streamRateCriticalPoint.toPercent()}%. Верните конвертер в вертикальное положение!
               """.trimIndent()){
    this.eventCritical()
}
